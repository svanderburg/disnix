...
static pid_t send_snapshot_mapping(SnapshotMapping *mapping, Target *target, const unsigned int flags)
{
    ...
    return exec_copy_snapshots_to((gchar*)target->client_interface, target_key, (gchar*)mapping->container, (gchar*)mapping->component, (flags & FLAG_ALL));
}
...
static pid_t restore_snapshot_on_target(SnapshotMapping *mapping, ManifestService *service, Target *target, xmlChar **arguments, unsigned int arguments_length)
{
    ...
    return exec_restore((char*)target->client_interface, target_key, (char*)mapping->container, (char*)service->type, (char**)arguments, arguments_length, (char*)service->pkg);
}
