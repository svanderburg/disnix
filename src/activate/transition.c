        ...
        case ACTIVATIONMAPPING_DEACTIVATED:
            {
                ...
                if(request_available_target_core(target)) /* Check if machine has any cores available, if not wait and try again later */
                {
                    ...
                    if(dry_run)
                        ...
                        pid = exec_activate(target->client_interface, actual_mapping->target, actual_mapping->container, actual_mapping->type, arguments, arguments_size, actual_mapping->service); /* Execute the activation operation asynchronously */
        ...
        case ACTIVATIONMAPPING_ACTIVATED:
            {
                ...
                else if(request_available_target_core(target))
                {
                    ...
                    if(dry_run)
                        ...
                        pid = exec_deactivate(target->client_interface, actual_mapping->target, actual_mapping->container, actual_mapping->type, arguments, arguments_size, actual_mapping->service); /* Execute the deactivation operation asynchronously */
