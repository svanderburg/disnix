...
typedef pid_t (*map_profilemanifesttarget_function) (void *data, gchar *target_key, ProfileManifestTarget *profile_manifest_target);

/** Pointer to a function that gets executed when a process completes for a profile manifest target */
typedef void (*complete_profilemanifesttarget_function) (void *data, ProfileManifestTarget *profile_manifest_target, ProcReact_Status status, int result);
