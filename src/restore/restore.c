...
static pid_t send_snapshot_mapping(SnapshotMapping *mapping, Target *target, int all)
{
    ...
    return exec_copy_snapshots_to(target->client_interface, mapping->target, mapping->container, mapping->component, all);
}
...
pid_t send_snapshots_to_target(void *data, Target *target)
{
    ...
    if(pid == 0)
    {
        ...
        for(i = 0; i < snapshots_per_target_array->len; i++)
        {
            ...
            exit_status = procreact_wait_for_exit_status(send_snapshot_mapping(mapping, target, send_snapshots_data->all), &status);
...
static pid_t send_restore_and_clean_snapshot_on_target(void *data, Target *target)
{
    ...
    if(pid == 0)
    {
        ...
        for(i = 0; i < snapshots_per_target_array->len; i++)
        {
            ...
            if(!procreact_wait_for_boolean(send_snapshot_mapping(mapping, target, send_snapshots_data->all), &status) || (status != PROCREACT_STATUS_OK)
...
static int restore_depth_first(GPtrArray *snapshots_array, GPtrArray *target_array, const unsigned int max_concurrent_transfers, const int all, const int keep)
{
    int success;
    SendRestoreAndCleanSnapshotsData data = { snapshots_array, all, keep };
...
int restore(const gchar *manifest_file, const unsigned int max_concurrent_transfers, const int transfer_only, const int depth_first, const int all, const int keep, const gchar *old_manifest, const gchar *coordinator_profile_path, gchar *profile, const gboolean no_upgrade, const gchar *container_filter, const gchar *component_filter)
{
    ...
    else
    {
        ...
        if(depth_first)
        {
            if(restore_depth_first(snapshots_array, manifest->target_array, max_concurrent_transfers, all, keep))
                exit_status = 0;
            else
                exit_status = 1;
        }
